...
python3 banned_words_checker.py --solution=$1.cpp --banned-words=banned_words.json
if [[ ! $? -eq 0 ]]
then
  echo "В вашем коде есть забаненые слова!"
  exit 1
...
clang-tidy -extra-arg=-std=c++23 -quiet $1.cpp
if [[ ! $? -eq 0 ]]
then
  echo "Проверка clang-tidy не пройдена"
  exit 1
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' $1.cpp &&
