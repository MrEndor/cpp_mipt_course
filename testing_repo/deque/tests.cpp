...
TEST(DequeConstructors, CopyNotEmpty) {
  size_t size = 17;
  int value = 14;
  Deque<int> original(size, value);
  EXPECT_TRUE(std::all_of(original.begin(),
                          original.end(),
                          [value = value](const auto& item) {
                            return item == value;
                          }));
  Deque<int> copy = original;
  EXPECT_TRUE(std::all_of(copy.begin(),
                          copy.end(),
                          [value = value](const auto& item) {
                            return item == value;
                          }));
  EXPECT_TRUE(CompareStacks(original, copy));
}
...
TEST(DequeModification, PushAndPop) {
  std::deque<NotDefaultConstructible> d(10000, {1});
